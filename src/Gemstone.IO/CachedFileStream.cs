...
namespace Gemstone.IO
{
    ...
    public class CachedFileStream : Stream
    {
        ...
        public override bool CanRead => m_fileStream.CanRead;
        ...
        /// </returns>
        /// <filterpriority>1</filterpriority>
        public override bool CanWrite => m_fileStream.CanWrite;
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            long end = Math.Min(m_position + count, Length);
            int totalCount = (int)(end - m_position);

            while (m_position < end)
            {
                Block block = GetBlock(m_position / BlockSize);
                int blockOffset = (int)(m_position % BlockSize);
                int blockCount = Math.Min((int)(end - m_position), BlockSize - blockOffset);
        ...
        private Block GetBlock(long blockIndex)
        {
            ...
            if (m_blockLookup.TryGetValue(blockIndex, out Block? block))
            {
                AddRef(block);

                if (m_queue.Count > 2L * m_cacheSize / BlockSize)
                    CleanQueue();
            ...
            PurgeCache(m_cacheSize - BlockSize);

            // Allocate a new buffer for the block
            byte[] buffer = new byte[BlockSize];

            if (blockIndex * BlockSize < m_fileStream.Length)
            {
                // Read the block from the file
                m_fileStream.Seek(blockIndex * BlockSize, SeekOrigin.Begin);
                m_fileStream.Read(buffer, 0, BlockSize);
            }
        ...
        private void PurgeCache(long cacheSize)
        {
            ...
            while (m_blockLookup.Count * BlockSize > cacheSize)
            {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (block.RefCount == 0)
                {
                    ...
                    if (bytesToEOF > 0)
                    {
                        ...
                        if (bytesToEOF < BlockSize)
                            Array.Clear(block.Buffer, (int)bytesToEOF, BlockSize - (int)bytesToEOF);
                    }
                    else
                    {
        ...
        {
            long blockPointer = block.BlockIndex * BlockSize;
            m_fileStream.Seek(blockPointer, SeekOrigin.Begin);
            m_fileStream.Write(block.Buffer, 0, Math.Min((int)(m_length - blockPointer), BlockSize));
        }
        ...
        private void RemoveRef(Block block) => block.RefCount--;
